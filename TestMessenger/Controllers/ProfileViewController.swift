...
class ProfileViewController: UIViewController {
    ...
    let nameLabel = UILabel(text: "Yurii Bilyar", font: .systemFont(ofSize: 20, weight: .light))
    ...
    @objc private func sendMessage() {
        ...
        self.dismiss(animated: true) {
            FirestoreService.shared.createWaitingChat(message: message, receiver: self.user) { (result) in
                ...
                case .success:
                    UIApplication.getTopViewController()?.showAlert(with: "Success!", and: "You message\(self.user.username) send.")
